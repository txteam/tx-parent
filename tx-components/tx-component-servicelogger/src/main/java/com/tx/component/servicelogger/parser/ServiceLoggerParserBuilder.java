 ...
 * @since  [产品/模块版本]
 */
public class ServiceLoggerParserBuilder {
    
    /** 日志对象实际不会太多，所以此处直接使用HashMap而不是WeakHashMap */
    @SuppressWarnings("rawtypes")
    private Map<Class<?>, ServiceLoggerParser> mapping = new WeakHashMap<Class<?>, ServiceLoggerParser>(TxConstants.INITIAL_MAP_SIZE);
    ...
    public <T> ServiceLoggerParser<T> parse(Class<T> type, Dialect dialect) {
        synchronized (type) {
            ServiceLoggerParser<T> parser = null;
            ...
            //简答的sqlSource源
            parser = new ServiceLoggerParser<T>(type, dialect);
            parser.setPkName(jpaMetaClass.getPkGetterName());
            parser.setTableName(jpaMetaClass.getTableName());
            
            //添加getter与字段的映射关系<br/>
            addGetter2ColumnMapping(jpaMetaClass, parser);
            
            //添加可更新字段
            addUpdateAblePropertys(type, jpaMetaClass, parser);
            //添加查询条件
            addQueryCondition(type, parser);
            
            //添加排序条件
            addOrderBy(type, jpaMetaClass, parser);
            
            //缓存起来
            mapping.put(type, parser);
            return parser;
        }
    }
    ...
    private <T> void addOrderBy(Class<T> type, JpaMetaClass<T> jpaMetaClass,
            ServiceLoggerParser<T> simpleSqlSource) {
...
//                queryConditionKeyType = getterTypeJpaMetaClass.getPkGetterType();
    ...
    private <T> void addUpdateAblePropertys(Class<T> type,
            JpaMetaClass<T> jpaMetaClass, ServiceLoggerParser<T> simpleSqlSource) {
