...
import com.tx.component.file.viewhandler.FileDefinitionResource;
...
public class FileContextHttpRequestHandler extends ResourceHttpRequestHandler {
    ...
    private FileContextImpl fileContext;
    ...
    protected final Resource getResource(HttpServletRequest request) {
            ...
            try {
                if (isInvalidPath(URLDecoder.decode(path, "UTF-8"))) {
                    if (logger.isTraceEnabled()) {
                        logger.trace(
                                "Ignoring invalid resource path with escape sequences ["
                                        + path + "].");
                    }
                    return null;
                }
        ...
        String[] paths = org.apache.commons.lang3.StringUtils
                .splitByWholeSeparator(path, "/");
        ...
        ViewHandler viewHandler = this.viewHandlerRegistry
                .getViewHandler(viewHandlerName);
     ...
     */
    public void setViewHandlerRegistry(
