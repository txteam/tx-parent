...
import com.tx.component.messagesender.context.AbstractMRSReceiver;
import com.tx.component.messagesender.context.DefaultResponse;
import com.tx.component.messagesender.context.MRSReceiver;
 ...
 * @since [产品/模块版本]
 */
@Component("mrs.receiver.alidayu.sms")
public class AlidayuSMSReceiver
        extends AbstractMRSReceiver<AlidayuSMSRequest, DefaultResponse<AlibabaAliqinFcSmsNumSendResponse>>
        implements MRSReceiver<AlidayuSMSRequest, DefaultResponse<AlibabaAliqinFcSmsNumSendResponse>>, BeanNameAware {
        
    /** 返回格式为 json */
    private static final String format = "json";
    
    /** 请求地址 */
    private static final String httpUrl = "http://gw.api.taobao.com/router/rest";
    ...
    @Override
    public void doHandle(AlidayuSMSRequest request, DefaultResponse<AlibabaAliqinFcSmsNumSendResponse> response) {
        ...
        List<Collection<String>> splitList = TxCollectionUtils.splitSize(set, SMS_REC_NUMS_MAX);
        for (Collection<String> collection : splitList) {
            smsRecNum = StringUtils.join(collection, ',');
        }
        
        TaobaoClient client = new DefaultTaobaoClient(httpUrl, appKey, appSecret, format);
        ...
        try {
            ...
            throw new MessageSenderException(e, "阿里大鱼-短信 api 调用失败 : {}", request.toString());
        }
    }
    ...
    protected void checkRequest(AlidayuSMSRequest request) {
        ...
        AssertUtils.notEmpty(request.getSmsFreeSignName(), "smsFreeSignName[短信签名] is empty!");
        AssertUtils.notEmpty(request.getSmsRecNums(), "smsRecNum[短信接收号码] is empty!");
        AssertUtils.notEmpty(request.getSmsTemplateCode(), "smsTemplateCode[短信模板ID] is empty!");
    }
    
    @Override
    protected void logHandle(AlidayuSMSRequest request, DefaultResponse<AlibabaAliqinFcSmsNumSendResponse> response, Throwable throwable) {
        AlibabaAliqinFcSmsNumSendResponse body = response.getBody();
        AlibabaAliqinFcSmsNumSendRequest req = response.getValue("_req", AlibabaAliqinFcSmsNumSendRequest.class);
        ...
        log.setUseTime(response.getValueOfNumber(super.RESPONSE_VALUE_USE_TIME) + "/毫秒");
        log.putOtherParam("SmsTemplateCode", request.getSmsTemplateCode());
        log.putOtherParam("RequestSourceName", request.getRequestSource().name());
        ...
        ServiceLoggerContext.logWithNotInitContext(log, TxLoaclFileServiceLog.class);
    }
}
