...
public class AuthContextTest {
    ...
    public void testRegisteAuth() {
        Assert.assertNotNull(authContext.registeAuth("test1", "测试权限1", "测试权限1", AuthConstant.AUTHTYPE_OPERATE, true));
        Assert.assertNotNull(authContext.registeAuth("test2", "测试权限2", "测试权限2", AuthConstant.AUTHTYPE_OPERATE, true));
    }
    ...
    public void saveAuthItemOfAuthRefIdList1(){
        ...
        AuthSessionContext.bindCurrentSessionToThread(new MockHttpServletRequest(), new MockHttpServletResponse());
        AuthSessionContext.putOperatorIdToSession("123456");
        authContext.saveAuthItemOfAuthRefIdList(AuthConstant.AUTHREFTYPE_OPERATOR, "test1", refIdList);
        
        List<AuthItemRef> resList1 = authContext.queryAuthItemRefListByAuthRefTypeAndRefId(AuthConstant.AUTHREFTYPE_OPERATOR, "111");
        Assert.assertTrue(resList1.size() > 0);
        List<AuthItemRef> resList2 = authContext.queryAuthItemRefListByAuthRefTypeAndAuthItemId(AuthConstant.AUTHREFTYPE_OPERATOR, "test1");
    ...
    public void saveAuthItemOfAuthRefIdList2(){
        ...
        authContext.saveAuthItemOfAuthRefIdList(AuthConstant.AUTHTYPE_OPERATE, AuthConstant.AUTHREFTYPE_OPERATOR, "test2", refIdList);
        
        List<AuthItemRef> resList1 = authContext.queryAuthItemRefListByAuthRefTypeAndRefId(AuthConstant.AUTHREFTYPE_OPERATOR, "1111");
        Assert.assertTrue(resList1.size() > 0);
        List<AuthItemRef> resList2 = authContext.queryAuthItemRefListByAuthRefTypeAndAuthItemId(AuthConstant.AUTHREFTYPE_OPERATOR, "test2");
        Assert.assertTrue(resList2.size() > 0);
