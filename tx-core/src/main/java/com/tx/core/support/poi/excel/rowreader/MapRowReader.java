 ...
 * @since  [产品/模块版本]
 */
public class MapRowReader implements CellRowReader<Map<String, String>> {
    ...
    /** <默认构造函数> */
    public MapRowReader(String[] keys) {
        super();
        AssertUtils.notEmpty(keys, "keys is empty.");
        
        this.keys = keys;
        this.cellReader = CellReaderBuilder.build(CellReader4StringValue.class,
                String.class);
    }
    
    /** <默认构造函数> */
    public MapRowReader(String[] keys, CellReader<String> cellReader) {
        super();
        AssertUtils.notEmpty(keys, "keys is empty.");
